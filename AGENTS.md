...
- The active homeserver is stored client-side in `localStorage` as `novarum:home-server`; localhost-style homeservers are forced to `http://`, others to `https://`.
- shadcn-svelte component config lives in `apps/frontend/components.json`; generated UI components go under `$lib/components/ui`.

## Backend (`apps/anchor`)
- Elysia/Bun backend entrypoint is `apps/anchor/src/index.ts`; it listens on port `5049` and exports `type App` for frontend Eden typing.
- Backend routes are mounted from `apps/anchor/modules`, not only `src`: `/.well-known/anchor/info` and `/auth/*` are wired in `src/index.ts`.
...
- Anchor uses Prisma Next with Postgres. `apps/anchor/prisma-next.config.ts` loads `.env` via `dotenv/config` and reads `DATABASE_URL`.
