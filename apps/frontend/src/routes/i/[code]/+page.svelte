<script lang="ts">
	...
	const inviteCode = $derived(rawInviteCode.startsWith('$') ? rawInviteCode.slice(1) : rawInviteCode);
	...
	async function acceptInvite() {
		...
		accepted = true;
		await goto(`/guilds/${result.data.guildId}`);
	}
		...
		{:else if error}
			...
			>
				...
				<Card.Content class="space-y-4">
					...
					<div class="flex items-center gap-2 rounded-sm border border-border bg-muted/50 px-3 py-2">
			...
			>
				<Card.Header class="space-y-3">
					...
					<div class="space-y-1">
						...
						<Card.Description>
							Join your friends on a shared server.
						</Card.Description>
					</div>
				</Card.Header>
					...
					>
						...
						<div class="min-w-0 flex-1">
							...
							<p class="flex items-center gap-1 text-xs text-muted-foreground">
								<Users class="size-3" />
								<span>{invite.guild.memberCount} member{invite.guild.memberCount !== 1 ? 's' : ''}</span>
