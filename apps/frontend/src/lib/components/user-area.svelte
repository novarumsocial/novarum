...
>
  ...
  <div class="min-w-0 flex-1">
    ...
    {:else}
      <p class="truncate text-sm font-medium leading-tight text-sidebar-foreground">{data?.user.displayName || data?.user.username}</p>
      <p class="truncate text-[11px] text-muted-foreground">@{data?.user.username}@{data?.user.homeserverName}</p>
    {/if}
  </div>
